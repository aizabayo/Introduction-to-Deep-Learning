...
class Attention:
        ...
        def forward(self, X):
            ...
            self.A_w    = torch.matmul(self.Q, self.K.permute(0, 2, 1)) / torch.sqrt(torch.tensor(self.W_k.size(1), dtype=torch.float32)) # TODO

            # Create additive causal attention mask and apply mask
            # Hint: Look into torch.tril/torch.triu and account for batch dimension

            attn_mask    = torch.tril(torch.ones(self.A_w.shape)).to(self.A_w.device) # TODO
            self.A_w = self.A_w.masked_fill(attn_mask == 0, float('-inf')) # Added TODO
        ...
        def backward(self, dLdXnew):
            ...
            self.dLdV      = torch.matmul(self.A_sig.permute(0, 2, 1), dLdXnew) # TODO
            self.dLdK      = torch.matmul(dLdA_w.permute(0, 2, 1), self.Q) # TODO
            ...
            self.dLdWq = torch.matmul(self.X.permute(0, 2, 1), self.dLdQ).sum(dim=0) # TODO
            self.dLdWv = torch.matmul(self.X.permute(0, 2, 1), self.dLdV).sum(dim=0) # TODO
            self.dLdWk = torch.matmul(self.X.permute(0, 2, 1), self.dLdK).sum(dim=0) # TODO

            # Derivative wrt input

            dLdX = (
                torch.matmul(self.dLdV, self.W_v.T) 
                + torch.matmul(self.dLdK, self.W_k.T) 
                + torch.matmul(self.dLdQ, self.W_q.T)
            ) # TODO
